# Copyright (c) 2020~2022 Qilu University of Technology, School of Computer Science & Technology, Class of Software Engineering(Software Development) 21-1, Duyu (No.202103180009).
...
def mixer(mixer_left, mixer_right, mixer_left_leftRate=1.0, mixer_left_rightRate=-1.0, mixer_right_leftRate=-1.0, mixer_right_rightRate=1.0):
    print("[STEPS]Mixing...")
...
def pitch(pitch_left, pitch_right, pitch_dataType, input_sampleRate, output_sampleRate, pitch_pitchFactor=0.8,
    ...
    current_sampleRate, resultLeft, resultRight = resample(int(input_sampleRate / pitch_pitchFactor), output_sampleRate, resultLeft, resultRight)
...
def showFrequencyDomainWave(sampleRate, waveData, fWave_dataType, fWave_fftSize=2048, offset=0, figSize=(8, 4)):
    print("[STEPS]Analysing Frequency Domain Wave...")
    typeFactor = getTypeFactor(fWave_dataType)
    xs = waveData[offset:fWave_fftSize + offset] / typeFactor
    xf = nprfft(xs) / fWave_fftSize
    freq = nplinspace(0, int(sampleRate / 2), int(fWave_fftSize / 2) + 1)
...
def showTimeDomainWave(sampleRate, waveData, timeWave_dataType, figSize=(8, 4), timeWave_xLabelUnit='second'):
    print("[STEPS]Analysing Time Domain Wave...")
    stdw_length = len(waveData)
    typeFactor = getTypeFactor(timeWave_dataType)
    figure("Time Domain Wave", figsize=figSize)
    if timeWave_xLabelUnit.lower() == 'second':
        plot(nparray(range(0, stdw_length)) / sampleRate, waveData / typeFactor)
        xlabel("Time(s)")
    else:
        plot(range(0, stdw_length), waveData / typeFactor)
        xlabel("Samples")
    ylabel("Amplitude")
    show()
...
if __name__ == '__main__':
    print("[INFOS]Copyright (c) 2020~2022 Qilu University of Technology, "
          "School of Computer Science & Technology, Duyu (No.202103180009).")
    print("[DESCR]Duyu WAV Audio Processor Core v3.3.0")
    filterwarnings('ignore')
    CommandLineArguments = argv  # We can use commandline to start this program.
    parser = ArgumentParser(epilog="Built and packaged with PyInstaller 5.1 __July 18, 2022.")
    parser.add_argument("--input", "-i", required=False, help="Input file name", nargs=1, dest="inputFileName", metavar="inputFile")
    parser.add_argument("--output", "-o", required=False, help="Output file name", nargs=1, dest="outputFileName", metavar="outputFile")
    parser.add_argument("--output-option", required=False, help="Setup sample rate and bit rate of output file.", nargs=2, dest="output_option", metavar=("sampleRate", "bitDepth"))
    parser.add_argument("--overwrite", required=False, action="store_true", dest="overwrite", help="Rewriting output file if it exists.")
    parser.add_argument("--reverb", required=False, help="Option of reverberation or echo, We suggest that these four parameters are 3, 2, 0.22, 12.0 (int,int,float,float).", nargs=4, dest="reverb", metavar=("numberOfEcho", "maxVolumeDb", "offsetSecond", "soundFieldVol"))
    parser.add_argument("--reverb-default", required=False, action="store_true", help="Using default parameter to execute reverb operation.", dest="reverb_default")
    parser.add_argument("--mix", required=False, help="Mix left and right channels as one. The four values should be a decimal from -1 to 1.", nargs=4, dest="mix", metavar=("left_leftRate", "left_rightRate", "right_leftRate", "right_rightRate"))
    parser.add_argument("--mix-default", action="store_true", required=False, help="Using default parameters to mix channels (Vocal Cut).", dest="mix_default")
    parser.add_argument("--gain", required=False, help="Gain or attenuator.", nargs=3, dest="gain", metavar=("valueLeft", "valueRight", "Unit"))
    parser.add_argument("--gain-default", required=False, help="Default gain operation: Gain for 3 Db.", action="store_true", dest="gain_default")
    parser.add_argument("--pitch", required=False, help="Modify the speed and pitch of the audio. We suggest these two values are decimals from 0.5 to 1.5", nargs=2, dest="pitch", metavar=("pitchFactor", "speedFactor"))
    parser.add_argument("--pitch-default", required=False, help="Using default parameter to execute pitch.", action="store_true", dest="pitch_default")
    parser.add_argument("--frequencyWave", required=False, help="show Frequency Domain Wave Picture.", nargs=2, dest="freq", metavar=("fftSize", "offsetSecond"))
    parser.add_argument("--timeWave", required=False, help="show Time Domain Wave Picture. xLabelUnit can be 'second' or 'sample'.", nargs=1, dest="time", metavar="xLabelUnit")
    parser.add_argument("--trim", required=False, help="Take a clip of the audio. Unit can be 'second' or 'sample'.", nargs=3, dest="trim", metavar=("start", "end", "Unit"))
    parser.add_argument("--joint", required=False, help="Joint(append or mix) two audio as one. Mode can be 'append' or 'mix'", nargs=3, dest="joint", metavar=("inputFile01", "inputFile02", "Mode"))
    parser.add_argument("--addMute", required=False, help="Add a piece of mute into the audio. Unit can be 'second' or 'sample', Mode can be 'insert' or 'override'", nargs=4, dest="addSilence", metavar=("start", "length", "Unit", "Mode"))
    parser.add_argument("--filter", required=False, help="Filter the output wav data. ", action="store_true", dest="filter")
    args = parser.parse_args()
    if len(sys.argv) == 1:
        print("[ERROR]Using command line argument to run this software. Add '-h' or '--help' for more help and usage.", file=stderr)
        exit(1)
    ...
    if args.joint is not None:
        ...
        SampleRate, left, right, dataType = joint(left, left02, right, right02,
                                                  SampleRate, SampleRate02, out_SampleRate, dataType, dataType02, out_DataType, joint_MODE=joint_mode)
    else:
    ...
    if args.reverb is not None:
        ...
        offsetSecond = int(args.reverb[2])
        soundFieldVol = int(args.reverb[3])
        if numberOfEcho > 20 or numberOfEcho < 1:
            print("[ERROR]Parameter 'numberOfEcho' should be from 1 to 20.", file=stderr)
            exit(1)
        left, right = reverb(SampleRate, left, right, reverb_numberOfEcho=numberOfEcho, reverb_maxVolumeDb=maxVolumeDb, reverb_offsetSecond=offsetSecond, CONSTANT_REVERB_RANGE=soundFieldVol)

    if args.reverb is None and args.reverb_default == True:
        left, right = reverb(SampleRate, left, right)

    if args.mix is not None:
        left_leftRate, left_rightRate, right_leftRate, right_rightRate = float(args.mix[0]), float(args.mix[1]), float(args.mix[2]), float(args.mix[3])
    ...
    if args.pitch is not None:
        ...
        SampleRate, left, right = pitch(left, right, dataType, SampleRate, out_SampleRate, pitch_pitchFactor=pitchFactor, pitch_speedFactor=speedFactor)
    ...
    if args.addSilence is not None:
        start, length, UNIT, MODE = float(args.addSilence[0]), float(args.addSilence[1]), args.addSilence[2], args.addSilence[3]
        UNIT = UNIT.lower()
        MODE = MODE.lower()
        ...
        if (not MODE == "insert") and (not UNIT == "override"):
            ...
            exit(1)
        left, right = addSilence(SampleRate, start, length, left, right, dataType, addSilence_UNIT=UNIT, addSilence_MODE=MODE)
    ...
    if args.freq is not None:
        fftSize, offsetSecond = int(args.freq[0]), int(args.freq[1])
        showFrequencyDomainWave(SampleRate, (left + right) / 2.0, dataType, fWave_fftSize=fftSize, offset=offsetSecond)
    if args.time is not None:
        xLabelUnit = args.time[0]
        xLabelUnit = xLabelUnit.lower()
        if (not xLabelUnit == "second") and (not xLabelUnit == "sample"):
            print("[ERROR]argument 'xLabelUnit' should be 'second' or 'sample'.", file=stderr)
            exit(1)
        showTimeDomainWave(SampleRate, (left + right) / 2.0, dataType, timeWave_xLabelUnit=xLabelUnit)

    if (args.freq is not None) or (args.time is not None):
        print("[WARNS]Draw an image, no output file.", file=stderr)
    else:
        ...
        writePcmWavData(outputWavFileName, left, right, SampleRate, dataType, args.filter)
